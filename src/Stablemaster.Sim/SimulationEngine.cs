    ...
    public List<WeeklyUpdate> AdvanceWeek(GameData data, IReadOnlyList<TrainingPlan> plans)
    {
        ...
        foreach (var horse in data.Horses)
        {
            ...
            updates.Add(new WeeklyUpdate
            {
                HorseId = horse.Id,
                HorseName = horse.Name,
                Summary = $"{plan.Focus} plan @ {plan.Intensity:F2}; fit {horse.Fitness:F1}, fatigue {horse.Fatigue:F1}, form {horse.CurrentForm:F1}."
            });
        }
    ...
    public RaceResult SimulateRace(Race race, IEnumerable<Horse> entries, IEnumerable<Staff> staff)
    {
        ...
        var scored = entries
            ...
            .Select(h =>
            {
                ...
                var conditionScore = h.Fitness * 0.2 - h.Fatigue * 0.18 + h.CurrentForm * 0.2 + h.Morale * 0.08;
                ...
                return new
                {
                    Horse = h,
                    Score = score,
                    Explanation = $"base={baseScore:F1}, condition={conditionScore:F1}, staff={trainerBonus + jockeyBonus:F1}, surface={surfaceSuitability:F1}, distancePenalty={distancePenalty:F1}, randomness={randomness:F1}"
                };
            })
